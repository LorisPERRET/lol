...
namespace UnitTestEF
{
    ...
    public class UnitTestSkin
    {
        ...
        [TestMethod]
        public async Task TestGetSkinsByChampion()
        {
            var connection = new SqliteConnection("DataSource=:memory:");
            connection.Open();
            var options = new DbContextOptionsBuilder<SqlLiteDbContext>()
            ...
            using (var context = new SqlLiteDbContext(options))
            {
                ...
                Assert.AreEqual(context.Skins.Count(), 1);
                //Act
                lesSkins = context.Skins.Where(c => c.Champion == itemChampion).ToList();
            }
        ...
        [TestMethod]
        public async Task TestGetNbSkinsByChampion()
        {
            var connection = new SqliteConnection("DataSource=:memory:");
            connection.Open();
            var options = new DbContextOptionsBuilder<SqlLiteDbContext>()
            ...
            using (var context = new SqlLiteDbContext(options))
            {
                ...
                Assert.AreEqual(context.Skins.Count(), 1);
                //Act
                nbSkins = await context.Skins.Where(c => c.Champion == itemChampion).CountAsync();
            }
